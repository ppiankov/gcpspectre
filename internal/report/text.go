...
func (r *TextReporter) Generate(data Data) error {
	...
	w.println("gcpspectre — GCP Resource Waste Report")
	w.println(strings.Repeat("=", 40))
	w.println("")

	if len(data.Findings) == 0 {
		w.println("No idle resources found.")
		w.println("")
	...
	tw2 := &errWriter{w: tw}
	tw2.printf("SEVERITY\tTYPE\tRESOURCE\tPROJECT\tZONE\tWASTE/MO\tMESSAGE\n")
	tw2.printf("--------\t----\t--------\t-------\t----\t--------\t-------\n")
	...
	for _, f := range data.Findings {
		...
		tw2.printf("%s\t%s\t%s\t%s\t%s\t$%.2f\t%s\n",
			f.Severity, f.ResourceType, name, f.Project, zone, f.EstimatedMonthlyWaste, f.Message)
	}
...
func writeTextSummary(w *errWriter, data Data) {
	...
	w.printf("Projects scanned:        %d\n", data.Summary.ProjectsScanned)
