...
```
go install github.com/ppiankov/gcpspectre/cmd/gcpspectre@latest
```
...
- Does not remediate or modify GCP resources — scan is read-only
- Does not store findings or manage a findings database
- Does not replace dedicated GCP resources monitoring — point-in-time security audit only
...
```bash
gcpspectre scan --output json | jq '.summary'
gcpspectre scan --output json | jq '.findings[] | select(.severity == "critical")'
```
